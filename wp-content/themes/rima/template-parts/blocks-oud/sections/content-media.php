...
<section id="<?= $unique_id; ?>" style="<?= $section_color; ?>" class="relative <?= $classes; ?>">
    <div class="flex items-center justify-center flex-col lg:min-h-[615px] 3xl:min-[725px] lg:container">
        <div class="container flex items-center min-h-[430px] lg:h-full lg:p-0 lg:m-0  <?= $reorder ? 'columns-2' : false; ?>">
            ...
            <div class="lg:z-10 w-full lg:max-w-[44%] xl:max-w-[500px] py-16">
                ...
                <?php if($description): ?>
                    <div style="<?= $description_color; ?>"><?= $description; ?></div>
                <?php endif; ?>

                <?php if($button_1['link'] !== '#' || $button_2['link'] !== '#'): ?>
                    <div class="btn-wrap flex-wrap mt-7">
        ...
        <?php if($media['type'] == 'image'): ?>
            <img srcset="<?= wp_get_attachment_image_srcset($media['ID']); ?>" alt="<?= $media['alt']; ?>" class="w-screen min-h-[300px] h-[350px] md:h-[380px] object-cover object-center lg:h-full lg:w-1/2 lg:ml-auto lg:absolute lg:top-0 lg:bottom-0 <?= $reorder ? 'lg:left-0' : 'lg:right-0'; ?>"></img>
        <?php elseif($media['type'] == 'video'): ?>
            <video src="<?= $media['url']; ?>" alt="<?= $media['alt']; ?>" class="w-screen min-h-[300px] h-[350px] md:h-[380px] object-cover object-center lg:h-full lg:w-1/2 lg:ml-auto lg:absolute lg:top-0 lg:bottom-0 <?= $reorder ? 'lg:left-0' : 'lg:right-0'; ?>" muted autoplay loop playsInline></video>
        <?php endif; ?>
    </div>
</section>
